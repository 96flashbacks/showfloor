...
void bhv_bowser_bomb_loop(void) { // s_kirai_event
...
void bhv_bowser_bomb_explosion_loop(void) { // s_kopakirai_fire
    struct Object *mineSmoke;
    
    cur_obj_scale((f32) o->oTimer / 14.0f * 9.0 + 1.0);

    if (((o->oTimer & 0x03) == 0) && (o->oTimer < 20)) {
...
void bhv_bowser_bomb_smoke_loop(void) { // s_kopakirai_smoke (modified)
    cur_obj_scale((f32) o->oTimer / 14.0f * 9.0 + 1.0);
    ...
    // The smoke doesn't lose opacity over time
