 ...
 * IMPORTANT NOTES:
 *
 *     1) This file could be optimised further, I am *sure* of it
 *     2) There may be one or two bugs with my attempt to axe all that leftover junk
 */
...
struct Painting *sInsideCastlePaintings[] = { &wf_painting, &lll_painting, &ccm_painting, &jrb_painting,
                                              NULL };
 ...
 */
void stop_other_paintings(s16 *idptr) {
    s16 index;
    s16 id = *idptr;

    index = 0;
    while (sInsideCastlePaintings[index] != NULL) {
        struct Painting *painting = segmented_to_virtual(sInsideCastlePaintings[index]);
 ...
 */
void painting_state(s8 state, struct Painting *painting, s8 xSource, s8 ySource, s8 resetTimer) {
    // make sure no other paintings are rippling
    stop_other_paintings(&painting->id);
 ...
 */
void wall_painting_proximity_idle(struct Painting *painting) {
    // Check for Mario triggering a ripple
    if (painting->floorEntered & RIPPLE_LEFT) {
        painting_state(PAINTING_RIPPLE, painting, NEAREST_4TH, MARIO_Y, RESET_TIMER);
    } else if (painting->floorEntered & RIPPLE_MIDDLE) {
        painting_state(PAINTING_RIPPLE, painting, NEAREST_4TH, MARIO_Y, RESET_TIMER);
    } else if (painting->floorEntered & RIPPLE_RIGHT) {
        painting_state(PAINTING_RIPPLE, painting, NEAREST_4TH, MARIO_Y, RESET_TIMER);
        ...
        painting_state(PAINTING_ENTERED, painting, NEAREST_4TH, MARIO_Y, RESET_TIMER);
    } else if (painting->floorEntered & ENTER_MIDDLE) {
        painting_state(PAINTING_ENTERED, painting, NEAREST_4TH, MARIO_Y, RESET_TIMER);
    } else if (painting->floorEntered & ENTER_RIGHT) {
        painting_state(PAINTING_ENTERED, painting, NEAREST_4TH, MARIO_Y, RESET_TIMER);
 ...
 */
void wall_painting_proximity_rippling(struct Painting *painting) {
 ...
 */
void wall_painting_continuous_idle(struct Painting *painting) {
    // Check for Mario triggering a ripple
    if (painting->floorEntered & RIPPLE_LEFT) {
        painting_state(PAINTING_RIPPLE, painting, MIDDLE_X, MIDDLE_Y, RESET_TIMER);
    } else if (painting->floorEntered & RIPPLE_MIDDLE) {
        painting_state(PAINTING_RIPPLE, painting, MIDDLE_X, MIDDLE_Y, RESET_TIMER);
    } else if (painting->floorEntered & RIPPLE_RIGHT) {
        painting_state(PAINTING_RIPPLE, painting, MIDDLE_X, MIDDLE_Y, RESET_TIMER);
 ...
 */
void wall_painting_continuous_rippling(struct Painting *painting) {
    if (painting->floorEntered & ENTER_LEFT) {
        painting_state(PAINTING_ENTERED, painting, NEAREST_4TH, MARIO_Y, DONT_RESET);
    } else if (painting->floorEntered & ENTER_MIDDLE) {
        painting_state(PAINTING_ENTERED, painting, NEAREST_4TH, MARIO_Y, DONT_RESET);
    } else if (painting->floorEntered & ENTER_RIGHT) {
        painting_state(PAINTING_ENTERED, painting, NEAREST_4TH, MARIO_Y, DONT_RESET);
 ...
 */
void wall_painting_update(struct Painting *painting) {
    if (painting->rippleTrigger == RIPPLE_TRIGGER_PROXIMITY) {
        switch (painting->state) {
            case PAINTING_IDLE:
                wall_painting_proximity_idle(painting);
                break;
            case PAINTING_RIPPLE:
                wall_painting_proximity_rippling(painting);
                break;
        }
    } else if (painting->rippleTrigger == RIPPLE_TRIGGER_CONTINUOUS) {
        switch (painting->state) {
            case PAINTING_IDLE:
                wall_painting_continuous_idle(painting);
                break;
            case PAINTING_RIPPLE:
                wall_painting_continuous_rippling(painting);
                break;
        }
    }
}
...
Gfx *geo_painting_draw(s32 callContext, struct GraphNode *node, UNUSED void *context) {
    ...
    s32 id = gen->parameter;
    Gfx *paintingDlist = NULL;
    struct Painting *painting = segmented_to_virtual(sInsideCastlePaintings[id]);
