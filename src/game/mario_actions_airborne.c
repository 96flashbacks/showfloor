...
s32 act_wall_kick_air(struct MarioState *m) {
    ...
    // No play_mario_jump_sound here, so it doesn't play when doing wall kicks
...
s32 act_hard_backward_air_kb(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_MARIO_HAUGH, MARIO_MARIO_SOUND_PLAYED);
    common_air_knockback_step(m, ACT_HARD_BACKWARD_GROUND_KB, ACT_HARD_BACKWARD_GROUND_KB, MARIO_ANIM_BACKWARD_AIR_KB,
                              -16.0f);
    return FALSE;
}
    ...
    switch (perform_air_step(m, 0)) {
        ...
        case AIR_STEP_HIT_WALL:
            if (m->wall != NULL) {
                ...
                // Game Zero shows Mario went into the diving animation for a frame at 11:17
                set_mario_animation(m, MARIO_ANIM_DIVE);
...
s32 act_riding_hoot(struct MarioState *m) {
    ...
    // No MARIO_ANIM_HANG_ON_CEILING, as seen in footage
    set_mario_animation(m, MARIO_ANIM_HANG_ON_OWL);

    // Velocity is not set to 0 to make sure the camera turning is closer to the references
